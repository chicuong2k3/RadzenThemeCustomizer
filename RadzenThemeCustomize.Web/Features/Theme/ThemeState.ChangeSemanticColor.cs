...
{
    public static class ChangeSemanticColorActionSet
    {
        ...
        public sealed class Handler : ActionHandler<Action>
        {
            ...
            {
                ThemeState.SemanticColors[action.Variable] = action.Value;
                ...
                await _themeManagerService.UpdateThemeAsync(new()
                {
                    ...
                    Properties =
                    {
                        {
                            action.Variable,
                            action.Variable.Contains("border") ? $"var(--rz-border-width) solid var(--{action.Value})" : $"var(--{action.Value})"
                        }
                    }
                });
